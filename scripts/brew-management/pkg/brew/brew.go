...
// InstallSimplePackages installs packages from simple configuration
func InstallSimplePackages(config *types.PackageSimple, options *types.InstallOptions) error {
	if err := utils.CheckPrerequisites(); err != nil {
		return err
	}

	// Install taps first
	if !options.SkipTaps && (options.TapsOnly || (!options.BrewsOnly && !options.CasksOnly && !options.MasOnly)) {
		for _, tap := range config.Taps {
			if options.DryRun {
				utils.PrintStatus(utils.Yellow, fmt.Sprintf("[DRY RUN] Would install tap: %s", tap))
				continue
			}
			if err := installTap(tap, options.Verbose); err != nil {
				utils.PrintStatus(utils.Red, fmt.Sprintf("Failed to install tap: %s - %v", tap, err))
			} else {
				utils.PrintStatus(utils.Green, fmt.Sprintf("Installed tap: %s", tap))
			}
		}
	}

	// Install brews
	if !options.SkipBrews && (options.BrewsOnly || (!options.TapsOnly && !options.CasksOnly && !options.MasOnly)) {
		for _, brew := range config.Brews {
			if options.DryRun {
				utils.PrintStatus(utils.Yellow, fmt.Sprintf("[DRY RUN] Would install brew: %s", brew))
				continue
			}
			if err := installBrew(brew, options.Verbose); err != nil {
				utils.PrintStatus(utils.Red, fmt.Sprintf("Failed to install brew: %s - %v", brew, err))
			} else {
				utils.PrintStatus(utils.Green, fmt.Sprintf("Installed brew: %s", brew))
			}
		}
	}

	// Install casks
	if !options.SkipCasks && (options.CasksOnly || (!options.TapsOnly && !options.BrewsOnly && !options.MasOnly)) {
		for _, cask := range config.Casks {
			if options.DryRun {
				utils.PrintStatus(utils.Yellow, fmt.Sprintf("[DRY RUN] Would install cask: %s", cask))
				continue
			}
			if err := installCask(cask, options.Verbose); err != nil {
				utils.PrintStatus(utils.Red, fmt.Sprintf("Failed to install cask: %s - %v", cask, err))
			} else {
				utils.PrintStatus(utils.Green, fmt.Sprintf("Installed cask: %s", cask))
			}
		}
	}

	// Install mas apps
	if !options.SkipMas && (options.MasOnly || (!options.TapsOnly && !options.BrewsOnly && !options.CasksOnly)) {
		for _, app := range config.MasApps {
			if options.DryRun {
				utils.PrintStatus(utils.Yellow, fmt.Sprintf("[DRY RUN] Would install mas app: %s", app.Name))
				continue
			}
			if err := installMas(app.Name, app.ID, options.Verbose); err != nil {
				utils.PrintStatus(utils.Red, fmt.Sprintf("Failed to install mas app: %s - %v", app.Name, err))
			} else {
				utils.PrintStatus(utils.Green, fmt.Sprintf("Installed mas app: %s", app.Name))
			}
		}
	}
